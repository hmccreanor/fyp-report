...
Choosing larger batch sizes may improve hardware utilization, but excessively heterogeneous batches composed can potentially also introduce fragmentation if block sizes are large relative to typical sequence lengths, thus limiting efficiency.

To explore this tradeoff, we varied two key parameters in vLLM’s scheduling configuration:

\begin{itemize}
  \item \texttt{max\_num\_batched\_tokens}, which sets the maximum number of input tokens passed into a single attention kernel. 
    This directly controls the overall compute workload per kernel call.
  \item \texttt{max\_num\_seqs}, which limits how many distinct requests can contribute tokens to that kernel. 
    This determines the diversity and potential fragmentation of the batch.
\end{itemize}
