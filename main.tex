...
\begin{comment}
  - So I think I need to reread + redo a lot of this based on how my project direction has changed
    - In particular I think the related work section is currently based more on the idea that I would implement my own runtime, rather than modify a runtime to better support a specific usecase 
  - Need to add sections explaining a PRM and the types of tree search
    - beam search
    - best of n
  - Maybe also speak about Prism (https://arxiv.org/pdf/2505.04021v1) as it has some related ideas around optimisations (specifically the whole memory size adjustment idea)
\end{comment}
We provide an overview of transformer architecture in \ref{subsection:llmarchitecture}, as well as key components of the inference pipeline in \ref{subsection:llminference}, before detailing existing work around LLM inference runtimes in \ref{section:relatedwork}. We also explain in greater detail the idea of test-time compute scaling and describe relevant techniques in \ref{subsection:testtimecompute}.
...
\subsection{Test Time Compute Scaling}\label{subsection:testtimecompute}
...
Best of N extends the idea of self-consistency to include some sort of external verifier.
This verifier can be any machine learning model but is typically some sort of transformer model.
In the basic best of N setup, this verifier scores all the solutions sampled from the LLM and the one with the highest score is output as the final response.
...
\chapter{Analysis}
\begin{comment}
  - Explain results and how they point to my optimisation
\end{comment}

\chapter{Optimisation}
\begin{comment}
  - Detail how my optimisation works and how I evaluated it.
\end{comment}
