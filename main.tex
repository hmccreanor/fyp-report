...
\begin{comment}
plan:
  - start by showing scaling dynamics in throughput between llm and prm
    - show plot that shows decode throughput scales as batch size increases
    - contrast with plot that shows prefill doesn't really increase as batch size increases
  - show some sort of histogram to show composition of llm and prm batches
    - ie most llm batches are comprised of prefill
    - most llm batches are scheduling far fewer tokens than they could

  - use this to justify that we can be doing extra work with the llm 

  - explore space of potential future work
    - could schedule more work that we know we will use
      - benefit is that we will use it!
      - downside is potential kv cache eviction
      - downside is also that this then becomes a prefill step, and so we lose intertoken latency
    - speculative work
      - in order to speculate on future nodes, we need to at least know the current node (which we are in the process of generating) so 
      - downside is maybe we speculate on the wrong thing and so have to throw it away
\end{comment}
